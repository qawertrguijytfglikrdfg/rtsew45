...
UUID=${UUID:-'a7050a43-09bd-460d-8fae-d2559c8b0fde'}
VMESS_WSPATH=${VMESS_WSPATH:-'/$UUID-vm'}
VLESS_WSPATH=${VLESS_WSPATH:-'/$UUID-vl'}
TROJAN_WSPATH=${TROJAN_WSPATH:-'/$UUID-tr'}
...
vmlink=vmess://$(echo -n "{\"v\":\"2\",\"ps\":\"repl\",\"add\":\"$URL\",\"port\":\"443\",\"id\":\"$UUID\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"$URL\",\"path\":\"$VMESS_WSPATH\",\"tls\":\"tls\"}" | base64 -w 0)
vllink="vless://"$UUID"@"$URL":443?encryption=none&security=tls&type=ws&host="$URL"&path="$VLESS_WSPATH"#codesandbox_vless"
trlink="trojan://"$UUID"@"$URL":443?security=tls&type=ws&host="$URL"&path="$TROJAN_WSPATH"#codesandbox_trojan"
...
echo "点击链接查看：https://$URL/$UUID.html" > /usr/local/xray/info
